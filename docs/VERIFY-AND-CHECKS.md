...
for provisioning mechanics (reconciliation table: §13).

Authored ARC 008 (2026-08-09). Prior to this document the "standing check-script rule" was cited
by arcs but had no written definition and no home on disk; `checks/` was empty and no check had
ever been written.
...
This is not a discipline to remember. §3 makes it structural: a component's installation
procedure lives *only* in its check, so there is nowhere else to put it.
...
## 6. Manifest — `checks/verify_manifest.json`

Owns **ordering, parallelism, and failure policy only**.
...
### 13.1 Checks owed under §1 (backlog)

None of these exist. Every one is an environment change already made:

`ib_async` pin · hardware UUID / `node_identity.json` · sealed credentials · `nix-trading.slice`
core pinning · systemd units · `trade_history` schema · Postgres · IB Gateway + Xvfb ·
`unattended-upgrades` · `.venv` · `git` · `python3`
