...
def _gate(loaded: LoadedCheck, ctx: Context) -> str:
    ...
    if loaded.load_error:
        return ""
    if loaded.privilege != ctx.privilege:
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("--privilege", choices=["user", "root"], default="user")
    parser.add_argument(
        "--maintenance",
        action="store_true",
...
if __name__ == "__main__":
    ...
    OUTCOME = run(Mode.VERIFY, Context(nix_home=_preamble.Path.cwd(), mode=Mode.VERIFY))
