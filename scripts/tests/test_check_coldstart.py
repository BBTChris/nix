...
def test_an_EMPTY_tree_is_CANNOT_MEASURE_and_was_a_PASS_before_this_guard(
    ...
    `sys.path` permanently, so `importlib.import_module("nixrisk.coldstart")`
    ...
    A PASS over an absent subject is exactly what §17 forbids — the property
    was proven against a different tree than the one under judgement.
    """
...
def test_the_provenance_guard_ACCEPTS_a_real_copied_tree(home: Path) -> None:
    ...
    assert str(home) in str(Path(module.coldstart.__file__ or "").resolve())
