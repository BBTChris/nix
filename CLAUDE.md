...
14. **A declared EXCLUSION is a guard with its re-owning ceiling lifted, and nothing else lifted.** `check_artifact_gate_coverage` may move an artifact out of the ceiling-guarded `rows` into an `exclusions` bucket that the re-owning ceiling does not judge — but only under a recorded `CHECK-A<n>` amendment naming the architect ruling, because the gate cannot tell an authorized move from a laundering one. An exclusion stays inside the one-way ratchet (silent growth and acquired-coverage are still FAILs), stays assigned under §0g, carries a written justification, and declares **exactly one of two dispositions**. **TEMPORARY** — a holding state, owned by a LIVE arc (a completed owner is Cannot-measure, so it cannot outlive its owner). **PERMANENT** (`CHECK-A11`) — a ratified accept that names NO owner and holds the verdict guarded no longer, because there is no arc that can discharge it: doctrine C.9 forbids a second instrument over a subject the suite already drives, and doctrine B.3 calls an owner that cannot pay furniture. **The permanent class pays for that exemption with a control the temporary class never had:** it must name `covered_by` witnesses under `scripts/tests/`, and every run resolves each one — a missing witness, a witness that does not name the artifact, an empty list, or this gate's own test module (which names every baseline path and so proves nothing about any of them) is a FAIL. The exemption is from the owner field, which measured nothing; it is not from measurement. Live under `CHECK-A8` (D3.104, the overdue-work case), `CHECK-A9` (D3.138, the instrument-blind-spot case: `gitenv.py`/`registry.py` are covered by tests with real can-fail controls, and a second check would be the duplicate instrument doctrine C.9 forbids), and `CHECK-A11` (D3.104 RESOLVED, ARC 052: the owner field had been walked ARC 030 → … → 052, the last six consecutive close-outs, each recording that it was arc-boundary maintenance and not progress — a debt whose payer does not exist, restated once per arc as if it did). Each amendment ENUMERATES its paths. **The live instance — how many artifacts and which arc owns them — is `checks/gate_coverage_baseline.json`'s own `exclusions` map, and is NOT restated here.** This sentence read "eight artifacts, owner ARC 032" from ARC 031 until ARC 034 and was wrong for three of those arcs: the owner walked ARC 032 → 033 → 035 while the text stood still, and `guard_owner_defect` reads the JSON, never this file. Directive 3, measured on this line.
