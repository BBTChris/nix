...
def test_the_control_passes_and_its_evidence_names_what_it_read(
    ...
    # modules — 19 in this tree. The literal is banked evidence; it moves when a
    ...
    # BUMPED 18 -> 19 by ARC 033 / Stage 1D, by the INTEGRATOR rather than by the
    # module's author: the sub-agent was terminated by a session cap inside this
    # very commit gate, so the tripwire fired and nobody was left to answer it.
    # The admission is `scripts/nixrisk/halt.py` (§12.5's HALT state machine — the
    # producer behind `gate.py`'s HaltFlagPort, which has been read as branch 0 on
    # every pass since ARC 028 with nothing implementing it). It declares no
    # order-port verb, sends nothing, carries no retry shape and imports nothing
    # banned; `check_order_path_bans` re-scanned the widened scope and reports 0
    # banned calls.
    #
    # FIFTH independent firing of this literal in one arc, from a fifth worktree
    # blind to the other four. Correct for THIS branch; the integration merge must
    # resolve to the figure the gate reports on the MERGED tree.
