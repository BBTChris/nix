...
def test_engine_imports_no_third_party_modules() -> None:
    ...
    proc = subprocess.run(
        [sys.executable, "-c",
         ...
         " import verify, nixverify.engine, nixverify.render;"
         " mods = [m for m in sys.modules"
         " if not m.startswith(('_', 'nixverify', 'verify'))];"
         " import sysconfig, pathlib;"
         " stdlib = pathlib.Path(sysconfig.get_paths()['stdlib']);"
         " bad = [m for m in mods if getattr(sys.modules[m], '__file__', None)"
...
def test_engine_source_never_reads_stdin() -> None:
    ...
    for path in (REPO / "scripts" / "verify.py", *(REPO / "scripts" / "nixverify").glob("*.py")):
        ...
        assert "input(" not in source, f"{path} reads stdin"
        assert "stdin" not in source, f"{path} references stdin"
