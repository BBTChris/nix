...
def _gate(loaded: LoadedCheck, ctx: Context) -> str:
    ...
    # No disruptive clause here on purpose. Disruptive gates the REPAIR, not the
    # inspection — skipping would discard the report along with the fix, and the
    # operator would not learn of the drift until the weekly window. _execute
    # downgrades such a check to VERIFY instead.
    return ""
...
def _execute(checks_dir: Path, name: str, ctx: Context) -> CheckResult:
    ...
    # A disruptive check outside the maintenance window runs, but only to look:
    # it reports the defect and is denied the ability to act on it.
    withheld = (
...
def stored_uuid(path: Path) -> str:
    ...
    The key is `primary_partition_uuid` because that is what install.sh:43
    actually writes. Do not invent a name here — deriving it from the writer
    is CLAUDE.md directive 3. Reading a key install.sh never writes would make
    this check report a correctly provisioned node as FAIL_NEEDS_OPERATOR
    forever, a failure no repair could satisfy.
    """
    if not path.is_file():
        return ""
    try:
...
# disruptive repairs are withheld here and deferred to the weekly root run (§8).
# Do not name the flag literally in this comment — the unit test asserts the
# flag's absence by substring, and prose mentioning it fails against itself.
