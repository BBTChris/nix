...
| D1.10 | pre-commit hook suite installed and each hook actually capable of failing | ARC 006 / ARC 010 | **still open, narrowed by ARC 017.** Per-hook can-fail now stands at **7 of 8 demonstrated, 1 partial**: `ruff-check`/`pylint`/`mypy`/`complexipy` D3.2 (ARC 015), `bandit (production)` D3.1 (ARC 010), `bandit (tests)` D3.6 (ARC 017), `pytest-affected` D3.4 (ARC 017); **`ruff-format` D3.5 is partial and stays open** — caught but named no site, and it repairs rather than reports. What remains owed for D1.10 itself is different from any D3 row: nothing *asserts* the suite is installed or that its hook set is intact. `pre-commit run` is invoked by a human or by the git hook; there is no gate that fails when `.pre-commit-config.yaml` loses a hook, when `core.hooksPath` is unset, or when the pinned `rev:` moves to an environment that scans nothing (ARC 017 measured a pre-ARC-010 bandit env still sitting in `~/.cache/pre-commit` that reproduces the original vacuum exactly). Non-vacuity per hook was captured ARC 017 and is recorded in the config header — but capturing it once by hand is not a gate |
...
| D3.5 | `ruff-format` | **OPEN — partial, deliberately not rounded up.** Owed since ARC 006 and unrecorded until ARC 017. FAIL-with-CONTROL run inside its own 37-file scope: verdict **CAUGHT BUT DID NOT NAME THE SITE**. It is a *repairer*, not a reporter — the hook entry is `ruff format --force-exclude`, `ruff format` itself exits 0 having rewritten the file, and the exit 1 comes from **pre-commit's** before/after tree hash as "files were modified by this hook". Three consequences: (i) it names no file and no line (`1 file reformatted, 36 files left unchanged`); (ii) the second run of the same hook over the same defect **passes**, because the gate consumed its own subject (failure mode #7); (iii) that signal is attributed by tree-diff, not causation — measured live in ARC 017, a *concurrent* write by another process made the **bandit** hook report "files were modified by this hook" though bandit never writes a file. **Honest classification: a formatter, not a gate.** A reporting configuration exists and was demonstrated (`ruff format --check` → exit 1, sha256 unchanged, names `<file>:1:1` with a diff); adopting it stops commits being auto-formatted, which is a behaviour change ARC 017 was not scoped to make | unassigned — discharges in whichever arc decides auto-format-vs-report; the demonstration is already banked |
